...
*	Date: 10/25/14
*	Version: 0.9
*	Version Notes: This is the full functioning version. With extra variables. 
*/
...
const unsigned char* get_IP(int); 
...
int main(int argc, char *argv[])
{
	...
	// TODO: Check to see which of these we still need 
	int sockfd;
	...
	//TODO: create data structure to hold hostnames
	...
	//TODO: Get all the hostnames and put them into the string with a '~'
	...
	else if (numbytes_rx > 5)
	{
		...
		else 
		{
			...
			{
				rx_confirmed.payload[y] = ntohl(rx_verify.extra[y]);
			}

			//rx_confirmed.payload[0] = rx_verify.extra[0];
			//rx_confirmed.payload[1] = rx_verify.extra[1];
...
/*
* This function takes an INT and converts it into an IP Address.
*
*/
/*
const unsigned char* get_IP(int input) 
{
	unsigned char *final = malloc(4);
	...
	IP[2] = (input >> 16) & 0xFF;
	IP[3] = (input >> 24) & 0xFF;
	printf ("FUNCTION: IP = %s\n", IP);

	strcpy(final, IP);

	printf ("FUNCTION: final = %s\n", final);
